...
function EventsCard({
	...
	*/
	...
	// TODO: Setup event tags to utilise badge colours

	const badgeClass = /*badgeColorByType[data.tags[0]] ?? */ "bg-[#DC003B]";

	return (
		<div className="relative">
			<div className={cn(eventsHolderStyles({ style }), "h-full")}>
				<div className="absolute pt-7">
					<p className={`${badgeClass} pt-2 pb-2 pl-4 pr-4 text-2xl font-bold`}>{data.tags.length > 0 ? data.tags[0] : "Unknown"}</p>
				</div>
				<div className="p-8 flex flex-col gap-2">
					...
					/>
					<p className="text-lg">{data.date.end?.split("T")[0]}</p>
